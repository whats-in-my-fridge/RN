...
export function FridgeItemRow({ item, onDelete }: Props) {
  ...
  return (
    <View className="flex-row items-center gap-3 bg-surface-card border border-stroke-default rounded-card px-[16px] py-[12px]">
      {/* 이름 + 부가 정보 */}
      ...
      {/* 신선도 뱃지 */}
      <View
        className="rounded-full px-[10px] h-[24px] items-center justify-center border"
      ...
      >
        <MaterialIcons name="close" size={12} color={tokens.color["content-secondary"]} />
      </Pressable>
    </View>
  );
}
