...
PR body를 아래 4개 섹션 순서로 작성한다. **반드시 한국어**, 모든 헤딩은 `###` 레벨로 통일한다.

**마크다운 작성 규칙 (반드시 준수)**
- 특수기호(● 등)나 스페이스바를 이용한 수동 들여쓰기를 절대 사용하지 않는다.
- 제목은 `###`, 목록은 `-` 기호만 사용한다.
- 각 줄 끝의 후행 공백(trailing spaces)을 전부 제거한다.
- `### 변경 파일 트리` 섹션은 반드시 ` ```text ` 펜스 코드블록으로 감싼다. 코드블록 밖에서 트리를 그리면 들여쓰기가 깨진다.

---

### `### 요약` 섹션

- 이 PR이 해결하는 문제 또는 제공하는 가치를 1~3개 불릿으로.
- "무엇을 추가했다" 나열이 아니라 "왜", "어떤 목적"으로 중심.
