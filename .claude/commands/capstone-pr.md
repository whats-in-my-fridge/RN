...
- **이전 브랜치와 같은 도메인(예: fridge, chat 등)을 연속으로 작업한 경우**, 아래 내용을 반드시 포함한다.
  - 말투는 반드시 `~했습니다.` 체로 작성한다.
  - 이전 PR(#N)이 아직 머지되지 않아 `develop...HEAD` diff에 이전 작업분이 누적되어 보일 수 있음을 리뷰어에게 알린다.
  - **이번 PR에서 새로 추가·수정한 파일**과 **이전 PR에서 이미 다룬 파일**을 구분해서 명시한다.
  - 이번 PR만의 변경을 정확히 보려면 특정 커밋 범위(`git diff <이전브랜치>...HEAD`)를 확인하도록 안내한다.
  - 가장 많이 변경된 파일(diff stat 기준 상위 1~2개)을 언급한다.
  - 예) "이전 #58 PR이 아직 머지 대기 중이므로 File Changes에 해당 작업분이 함께 표시됩니다. 이번 #60의 실제 변경은 `feat/#58-fridge-entities-layer...HEAD` 범위를 확인해 주세요. 이번 PR에서 새로 작성된 파일은 `FridgeBoard.tsx`, `FridgePage.tsx`이며, `ShelfRow.tsx`·`DualShelfRow.tsx`는 D-임박 badge 추가로 일부 수정되었습니다. 이전 #58에서 신규 생성된 `types.ts`, `ItemChip.tsx` 등은 이번 PR 범위가 아닙니다."
